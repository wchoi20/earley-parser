...
class EarleyChart:
    ...
    def accepted(self) -> bool:
        ...
        for item in self.cols[-1].all():  # the last column
            if (item[0].rule.lhs == self.grammar.start_symbol  # a ROOT item in this column
                ...
                logging.debug(f"weight is {item[2]}")
                self.best_parse = item
                self.best_weight = item[2]
                return True
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        if position in self.batch.keys():
            self.batch[position].append(nonterminal)
        else:
            self.batch[position] = [nonterminal]
    ...
    def _scan(self, item: Item, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == item[0].next_symbol():
            ...
            self.cols[position + 1].push(new_item, (None, item[0], None, position), item[2])
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer[0].next_symbol() == item[0].rule.lhs:
                ...
                self.cols[position].push(new_item, (customer[0], item[0], mid, position), item[2] + customer[2])
    ...
    def print_item(self, item: Item, string_array):

        if item[1][0]:
            start = item[1][0].start_position
            dot = item[1][0].dot_position
            temp_key = Item(item[1][0].rule, dot_position=dot, start_position=start)
            temp_item = self.cols[item[1][2]].get_item(self.cols[item[1][2]].get_index(temp_key))
            self.print_item(temp_item, string_array)
            string_array.append(' ')

        if item[1][1] and item[1][1].dot_position > 0:
            start = item[1][1].start_position
            dot = item[1][1].dot_position
            temp_key = Item(item[1][1].rule, dot_position=dot, start_position=start)
            temp_item = self.cols[item[1][3]].get_item(self.cols[item[1][3]].get_index(temp_key))
            self.print_item(temp_item, string_array)
            if item[1][1].dot_position == len(item[1][1].rule.rhs):
                string_array.append(')')

        # no customer found
        if item[1][0] is None and item[1][1] is None:
            string_array.append('(')
            string_array.append(item[0].rule.lhs)

        if item[1][0] is None and item[1][1]:

            if item[1][1].dot_position == 0:
                string_array.append('(')
                string_array.append(item[0].rule.lhs)
                string_array.append(' ')

            i = item[1][1].dot_position
            string_array.append(item[0].rule.rhs[i])
            string_array.append(' ')

            if item[0].dot_position == 0:
                string_array.pop()
                string_array.append(')')
...
class Agenda:
    ...
    def __init__(self) -> None:
        self._items: List[(Item, [(Item, Item, int, int)], float)] = []  # list of all items that were *ever* pushed
    ...
    def push(self, item, backpointer_pair: (Item, Item, int, int), weight: float) -> None:
        """Add (enqueue) the item, unless it was previously added."""
        if item not in self._index:  # O(1) lookup in hash table
            self._items.append(tuple([item, backpointer_pair, weight]))
            self._index[item] = len(self._items) - 1

        elif item in self._index and weight < self._items[self._index.get(item)][2]:
            self._items[self.get_index(item)] = tuple([item, backpointer_pair, weight])
            self.reprocess_list.append(self._index[item])

    def update(self, item, backpointer_pair: (Item, Item, int, int), weight: float) -> None:

        self._items[self.get_index(item)] = tuple([item, backpointer_pair, weight])
        self.reprocess_list.append(self.get_index(item))
    ...
    def pop(self) -> Item:
        ...
        if len(self.reprocess_list) != 0:
            return self._items[self.reprocess_list.pop(0)]

        item = self._items[self._next]
        self._next += 1
        return item

    def all(self) -> Iterable[Item, (Item, Item, int, int), float]:
    ...
    def get_item(self, index: int) -> tuple[(Item, [(Item, Item, int, int)], float)]:
        return self._items[index]

    def had(self, item: Item) -> bool:
        return item in self._index

    def get_weight(self, item: Item) -> float:
        return self._items[self.get_index(item)][2]
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                # print the result
                if chart.accepted():
                    print(f"{''.join(chart.print_item(chart.get_best_item(), output_list))})")
                    print(f"{chart.get_best_item()[2]}")
                else:
                    print("NONE")
